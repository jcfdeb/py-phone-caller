...
def get_asterisk_callerid():
    """Returns the Asterisk user, can be retrieved from env var or configuration file."""
    return Config.get_var_value(
        env_var="ASTERISK_CALLERID",
        conf_table="asterisk_call",
        conf_value="asterisk_callerid",
    )


def call_register_http_scheme():
...
def get_call_register_app_route_register_call():
    """Returns the the path or route where the 'call_register' is waiting for a
...
def get_call_register_app_route_heard():
    """Returns the the path or route where the 'call_register' is waiting for a
    HTTP GET from the Asterisk PBX in order for register, into the DB, if the
    audio message was played to the calle (datetime). Example value: 'heard'
    Note: use a single string 'correct' and not something this '/not/correct'"""
    return Config.get_var_value(
...
def get_serving_audio_folder():
    """Returns the the folder where the audio files are placed in the 'generate_audio',
    can be retrieved from env var or configuration file."""
    return Config.get_var_value(
...
def get_asterisk_call_app_route_asterisk_init():
    """Returns the the path or route where the 'asterisk_call' is waiting for a
    HTTP POST in order to start a call against the Asterisk PBX.
    Example value: 'asterisk_init'
    Note: use a single string 'correct' and not something this '/not/correct'"""
    return Config.get_var_value(
        env_var="ASTERISK_CALL_APP_ROUTE_ASTERISK_INIT",
        conf_table="asterisk_call",
        conf_value="asterisk_call_app_route_asterisk_init",
    )
...
def get_generate_audio_app_route():
    """Returns the the path or route where the 'generate_audio' is waiting for a
...
def get_caller_sms_app_route():
    """Returns the the path or route where the 'caller_sms' is waiting for a
...
def get_log_formatter():
    """Returns the the format for the log 'Formatter(fmt=<The value configured in 'caller_config.toml'>)',
    can be retrieved from env var or configuration file."""
    return Config.get_var_value(
...
def get_registercall_error():
    """Returns a custom text error for the '/' endpoint of 'call_register',
    can be retrieved from env var or configuration file."""
    return Config.get_var_value(
        env_var="REGISTERCALL_ERROR",
        conf_table="logger",
        conf_value="registercall_error",
    )
...
class Config:
    ...
    def get_path_to_conf():
        try:
            ...
            logging.info(
                f"No environmental variable 'CALLER_CONFIG' found for the configuration file."
            )
            logging.info(f"Using the default path 'config/caller_config.toml'")
    ...
    def get_var_value(
        ...
        """Returns the the value from a environmental variable or configuration file
        ...
        try:
            ...
            logging.info(f"No values values passed.")
        try:
